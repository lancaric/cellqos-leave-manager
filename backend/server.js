...
import { computeAnnualLeaveAllowanceHours, computeCarryOverHours, getAnnualLeaveGroupAllowanceHours, } from "./shared/leave-entitlement";
...
app.use((req, _res, next) => {
    ...
    try {
        const payload = jwt.verify(token, jwtSecretValue);
        req.auth = {
            userID: payload.sub,
            email: payload.email,
            role: payload.role,
...
async function queryRow(text, values = []) {
...
async function createAuditLog(actorUserId, action, details) {
    await pool.query(`
...
async function createEntityAuditLog(actorUserId, entityType, entityId, action, before, after) {
    await pool.query(`
...
async function getVacationPolicy() {
...
async function resolveStatsScope(auth, requestedTeamId, memberIds) {
    const isAdminUser = isAdmin(auth.role);
    const isManagerUser = isManager(auth.role);
    let teamId = requestedTeamId;
    let teamName = null;
    if (!isAdminUser) {
        const viewer = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [auth.userID]);
        if (!viewer) {
    ...
    if (teamId) {
        const team = await queryRow("SELECT name FROM teams WHERE id = $1", [teamId]);
        teamName = team?.name ?? null;
    }
    const teamConditions = ["is_active = true"];
    const teamParams = [];
    if (teamId) {
    ...
    const scopedMembers = memberIds.length > 0
...
async function getUserColumnSupport() {
...
async function getVacationPolicySupport() {
...
async function getAnnualLeaveAllowanceHoursForUser(userId, year, skipCarryOver) {
    ...
    const overrideBalance = await queryRow(`
    ...
    const previousBalance = await queryRow(`
      SELECT allowance_hours as "allowanceHours"
      FROM leave_balances
      WHERE user_id = $1
        AND year = $2
...
async function ensureSlovakHolidaysForYear(year, actorUserId) {
    ...
    for (const seed of seeds) {
      ...
      `, [seed.date, seed.name, seed.isCompanyHoliday, true]);
        if (holiday) {
            await createEntityAuditLog(actorUserId, "holidays", holiday.id, "CREATE", null, holiday);
        }
    }
}
...
async function createNotification(userId, type, payload, dedupeKey) {
...
async function applyLeaveBalanceOverride(userId, overrideHours, actorUserId) {
    ...
    await createEntityAuditLog(actorUserId, "leave_balances", `${userId}-${currentYear}`, "OVERRIDE", {
        userId,
        year: currentYear,
    ...
    await createNotification(actorUserId, "leave_balance_override", {
        userId,
        year: currentYear,
        remainingBefore,
        remainingAfter,
...
async function getShowTeamCalendarForEmployees() {
    try {
        const settings = await queryRow("SELECT show_team_calendar_for_employees as \"showTeamCalendarForEmployees\" FROM settings LIMIT 1", []);
...
async function resetSerialSequence(client, table, column) {
    await client.query(`
      SELECT setval(
...
app.post("/auth/register", asyncHandler(async (req, res) => {
    const { email, name, password, teamId } = req.body;
    validateEmail(email);
    const existing = await queryRow("SELECT id FROM users WHERE email = $1", [email]);
    if (existing) {
...
app.get("/users", asyncHandler(async (req, res) => {
    ...
    const balanceRows = await queryRows(`
      ...
      GROUP BY user_id
    `, [year]);
    const bookedByUserId = new Map(balanceRows.map((row) => [row.userId, Number(row.total ?? 0)]));
...
app.post("/users", asyncHandler(async (req, res) => {
    ...
    const columnSupport = await getUserColumnSupport();
    const { email, name, role, teamId, birthDate, hasChild, employmentStartDate, manualLeaveAllowanceHours } = req.body;
    if (!email || !name) {
    ...
    const passwordRow = await queryRow("SELECT crypt($1, gen_salt('bf')) as hash", [defaultPassword]);
    ...
    await createEntityAuditLog(auth.userID, "users", userId, "CREATE", null, user);
    if (manualLeaveAllowanceHours !== undefined && manualLeaveAllowanceHours !== null) {
...
app.patch("/users/:id", asyncHandler(async (req, res) => {
    ...
    const { id } = req.params;
    const columnSupport = await getUserColumnSupport();
    const { email, name, role, teamId, isActive, birthDate, hasChild, employmentStartDate, manualLeaveAllowanceHours } = req.body;
    const before = await queryRow("SELECT * FROM users WHERE id = $1", [id]);
    if (!before) {
        throw new HttpError(404, "User not found");
    }
    if (email !== undefined) {
    ...
    await createEntityAuditLog(auth.userID, "users", id, "UPDATE", before, user);
    if (manualLeaveAllowanceHours !== undefined && manualLeaveAllowanceHours !== null) {
...
app.post("/users/:id/reset-password", asyncHandler(async (req, res) => {
    ...
    await pool.query(`
      ...
      WHERE id = $2
    `, [defaultPassword, id]);
    const adminUser = await queryRow(`
      SELECT name, email
      FROM users
      WHERE id = $1
    `, [auth.userID]);
...
app.delete("/users/:id", asyncHandler(async (req, res) => {
    ...
    const leaveRequestCount = await queryRow("SELECT COUNT(*) as count FROM leave_requests WHERE user_id = $1", [id]);
    const leaveBalanceCount = await queryRow("SELECT COUNT(*) as count FROM leave_balances WHERE user_id = $1", [id]);
    const notificationCount = await queryRow("SELECT COUNT(*) as count FROM notifications WHERE user_id = $1", [id]);
...
app.post("/teams", asyncHandler(async (req, res) => {
    const auth = requireAuth(req.auth ?? null);
    requireAdmin(auth.role);
    const { name, maxConcurrentLeaves } = req.body;
    if (!name) {
    ...
    await createEntityAuditLog(auth.userID, "teams", team.id, "CREATE", null, team);
    res.json(team);
}));
app.patch("/teams/:id", asyncHandler(async (req, res) => {
    ...
    const { name, maxConcurrentLeaves } = req.body;
    const before = await queryRow("SELECT * FROM teams WHERE id = $1", [id]);
    if (!before) {
    ...
    await createEntityAuditLog(auth.userID, "teams", id, "UPDATE", before, team);
    res.json(team);
}));
app.delete("/teams/:id", asyncHandler(async (req, res) => {
    ...
    const userCount = await queryRow("SELECT COUNT(*) as count FROM users WHERE team_id = $1", [id]);
...
app.post("/holidays", asyncHandler(async (req, res) => {
    const auth = requireAuth(req.auth ?? null);
    requireAdmin(auth.role);
    const { date, name, isCompanyHoliday, isActive } = req.body;
    if (!date || !name) {
    ...
    await createEntityAuditLog(auth.userID, "holidays", holiday.id, "CREATE", null, holiday);
    res.json(holiday);
}));
app.patch("/holidays/:id", asyncHandler(async (req, res) => {
    ...
    const id = Number(req.params.id);
    const { date, name, isCompanyHoliday, isActive } = req.body;
    const before = await queryRow("SELECT * FROM holidays WHERE id = $1", [id]);
    if (!before) {
    ...
    if (updates.length > 0) {
        values.push(id);
        try {
            await pool.query(`UPDATE holidays SET ${updates.join(", ")} WHERE id = $${values.length}`, values);
        }
        catch (error) {
            if (error instanceof Error && error.message.includes("duplicate key")) {
                throw new HttpError(409, "Holiday for this date already exists");
    ...
    await createEntityAuditLog(auth.userID, "holidays", id, "UPDATE", before, holiday);
    res.json(holiday);
}));
...
app.get("/leave-requests", asyncHandler(async (req, res) => {
    ...
    if (userId) {
        ...
        if (isManagerUser && !isAdminUser && userId !== auth.userID) {
            const target = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [userId]);
    ...
    `;
    const requests = await queryRows(query, values);
    const currentYear = new Date().getFullYear();
    const bookedRows = await queryRows(`
      SELECT user_id as "userId",
        COALESCE(SUM(computed_hours), 0) as total
      FROM leave_requests
      WHERE type = 'ANNUAL_LEAVE'
      ...
      GROUP BY user_id
    `, [currentYear]);
    const bookedByUserId = new Map(bookedRows.map((row) => [row.userId, Number(row.total ?? 0)]));
    const requestsWithBalance = await Promise.all(requests.map(async (request) => {
...
app.post("/leave-requests", asyncHandler(async (req, res) => {
    ...
    const isAdminUser = isAdmin(auth.role);
    const isManagerUser = isManager(auth.role);
    const { type, startDate, endDate, startTime, endTime, reason, userId, } = req.body;
    const targetUserId = userId ?? auth.userID;
    if (targetUserId !== auth.userID) {
        requireManager(auth.role);
        if (isManagerUser && !isAdminUser) {
            const viewer = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [auth.userID]);
            const target = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [targetUserId]);
    ...
    const userExists = await queryRow("SELECT id FROM users WHERE id = $1 AND is_active = true", [targetUserId]);
    if (!userExists) {
        throw new HttpError(404, "User not found");
    }
    validateDateRange(startDate, endDate);
    validateNotInPast(startDate, { allowPast: isManagerUser || isAdminUser });
    const holidayRows = await queryRows(`
      SELECT date::text as date FROM holidays
      WHERE date >= $1 AND date <= $2
        AND is_active = true
    `, [startDate, endDate]);
    const holidayDates = new Set(holidayRows.map((h) => h.date));
    const computedHours = computeWorkingHours(startDate, endDate, holidayDates, startTime ?? null, endTime ?? null);
    const result = await queryRow(`
    ...
    const leaveRequest = await queryRow(`
      SELECT 
      ...
      WHERE id = $1
    `, [result?.id]);
    await createEntityAuditLog(auth.userID, "leave_request", leaveRequest.id, "CREATE", null, leaveRequest);
...
app.patch("/leave-requests/:id", asyncHandler(async (req, res) => {
    ...
    const isManagerUser = isManager(auth.role);
    const id = Number(req.params.id);
    const { type, startDate, endDate, startTime, endTime, reason, managerComment, } = req.body;
    ...
    if (startDate ||
      ...
      `, [newStartDate, newEndDate]);
        const holidayDates = new Set(holidayRows.map((h) => h.date));
        computedHours = computeWorkingHours(newStartDate, newEndDate, holidayDates, startTime ?? before.startTime, endTime ?? before.endTime);
    }
    const updates = [];
    const values = [];
    if (type !== undefined) {
    ...
    const after = await queryRow(`
      ...
      WHERE id = $1
    `, [id]);
    await createEntityAuditLog(auth.userID, "leave_request", id, "UPDATE", before, after);
    if ((isManagerUser || isAdminUser) && before.userId !== auth.userID) {
        await createNotification(before.userId, "REQUEST_UPDATED_BY_MANAGER", {
            requestId: id,
...
app.delete("/leave-requests/:id", asyncHandler(async (req, res) => {
    ...
    await createEntityAuditLog(auth.userID, "leave_request", id, "DELETE", request, null);
    res.json({ ok: true });
}));
app.post("/leave-requests/:id/submit", asyncHandler(async (req, res) => {
    ...
    const requester = await queryRow("SELECT team_id as \"teamId\", name FROM users WHERE id = $1", [request.userId]);
    if (isManagerUser && !isAdminUser && request.userId !== auth.userID) {
        const viewer = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [auth.userID]);
    ...
    const updated = await queryRow(`
      ...
      WHERE id = $1
    `, [id]);
    await createEntityAuditLog(auth.userID, "leave_request", id, "SUBMIT", request, updated);
    const managers = requester?.teamId
        ? await queryRows("SELECT id FROM users WHERE role = 'MANAGER' AND is_active = true AND team_id = $1", [requester.teamId])
        : await queryRows("SELECT id FROM users WHERE role = 'MANAGER' AND is_active = true", []);
    for (const manager of managers) {
        await createNotification(manager.id, "NEW_PENDING_REQUEST", {
            requestId: id,
...
app.post("/leave-requests/:id/approve", asyncHandler(async (req, res) => {
    ...
    const updated = await queryRow(`
      ...
      WHERE id = $1
    `, [id]);
    await createEntityAuditLog(auth.userID, "leave_request", id, bulk ? "BULK_APPROVE" : "APPROVE", request, updated);
    await createNotification(request.userId, "REQUEST_APPROVED", {
        requestId: id,
        type: updated?.type,
        startDate: updated?.startDate,
        endDate: updated?.endDate,
...
app.post("/leave-requests/:id/reject", asyncHandler(async (req, res) => {
    ...
    const updated = await queryRow(`
      ...
      WHERE id = $1
    `, [id]);
    await createEntityAuditLog(auth.userID, "leave_request", id, bulk ? "BULK_REJECT" : "REJECT", request, updated);
    await createNotification(request.userId, "REQUEST_REJECTED", {
        requestId: id,
        type: updated?.type,
        startDate: updated?.startDate,
        endDate: updated?.endDate,
...
app.post("/leave-requests/:id/cancel", asyncHandler(async (req, res) => {
    ...
    const updated = await queryRow(`
      ...
      WHERE id = $1
    `, [id]);
    await createEntityAuditLog(auth.userID, "leave_request", id, "CANCEL", request, updated);
    const managers = requester?.teamId
        ? await queryRows("SELECT id FROM users WHERE role = 'MANAGER' AND is_active = true AND team_id = $1", [requester.teamId])
        : await queryRows("SELECT id FROM users WHERE role = 'MANAGER' AND is_active = true", []);
    for (const manager of managers) {
        await createNotification(manager.id, "REQUEST_CANCELLED", {
            requestId: id,
            userId: request.userId,
            userName: requester?.name,
            type: updated?.type,
...
app.get("/calendar", asyncHandler(async (req, res) => {
    ...
    if (!isAdminUser) {
        const viewer = await queryRow("SELECT team_id as \"teamId\" FROM users WHERE id = $1", [viewerId]);
        if (!viewer) {
            throw new HttpError(404, "User not found");
        }
    ...
    `;
    const events = await queryRows(query, values);
...
app.get("/stats/dashboard", asyncHandler(async (req, res) => {
    ...
    requireManager(auth.role);
    const currentYear = new Date().getFullYear();
    const year = parseOptionalNumber(req.query.year) ?? currentYear;
    const month = parseOptionalNumber(req.query.month);
    const quarter = parseOptionalNumber(req.query.quarter);
    const teamId = parseOptionalNumber(req.query.teamId);
    const memberIds = parseCsvList(req.query.memberIds);
    const eventTypesRaw = parseCsvList(req.query.eventTypes);
    ...
    const trend = range.months.map((monthValue) => {
        return (trendMap.get(monthValue) ?? {
            month: monthValue,
            totalEvents: 0,
            totalDays: 0,
        });
...
app.get("/stats/table", asyncHandler(async (req, res) => {
    ...
    const page = Math.max(1, parseOptionalNumber(req.query.page) ?? 1);
    const pageSize = Math.min(50, Math.max(5, parseOptionalNumber(req.query.pageSize) ?? 10));
    const sortBy = req.query.sortBy ?? "totalDays";
...
app.post("/stats/exports", asyncHandler(async (req, res) => {
    ...
    const allowedReportTypes = ["DASHBOARD_SUMMARY", "TABLE_DETAIL", "YEAR_CALENDAR"];
...
app.get("/stats/exports/:id/download", asyncHandler(async (req, res) => {
    ...
    res.setHeader("Content-Disposition", `attachment; filename=stats-export-${job.id}.${job.format.toLowerCase()}`);
...
app.get("/audit", asyncHandler(async (req, res) => {
    ...
    if (!isAdminUser && !isManagerUser) {
        ...
        const owned = await queryRow("SELECT id FROM leave_requests WHERE id = $1 AND user_id = $2", [Number(entityId), auth.userID]);
        if (!owned) {
            throw new HttpError(403, "Not allowed to view audit logs");
        }
    }
    ...
    const query = `
      ...
      LIMIT ${limit}
    `;
    const logs = await queryRows(query, values);
    res.json({ logs });
}));
...
app.post("/notifications/:id/read", asyncHandler(async (req, res) => {
    ...
    if (isAdmin(auth.role)) {
        ...
        await pool.query("UPDATE notifications SET read_at = NOW() WHERE id = $1 AND user_id = $2", [id, auth.userID]);
    }
    res.json({ ok: true });
}));
app.post("/notifications/read-all", asyncHandler(async (req, res) => {
    ...
    if (isAdmin(auth.role)) {
        ...
        await pool.query("UPDATE notifications SET read_at = NOW() WHERE user_id = $1 AND read_at IS NULL", [auth.userID]);
    }
    res.json({ ok: true });
}));
...
app.patch("/admin/vacation-policy", asyncHandler(async (req, res) => {
    ...
    const carryOverEnabled = typeof payload.carryOverEnabled === "boolean" ? payload.carryOverEnabled : before.carryOverEnabled;
    const carryOverLimitHours = typeof payload.carryOverLimitHours === "number" ? payload.carryOverLimitHours : before.carryOverLimitHours;
...
app.get("/admin/database/export", asyncHandler(async (req, res) => {
    ...
    requireAdmin(auth.role);
    const tables = {
        teams: await queryRows("SELECT * FROM teams ORDER BY id ASC"),
        users: await queryRows("SELECT * FROM users ORDER BY id ASC"),
        leave_requests: await queryRows("SELECT * FROM leave_requests ORDER BY id ASC"),
        holidays: await queryRows("SELECT * FROM holidays ORDER BY id ASC"),
        leave_balances: await queryRows("SELECT * FROM leave_balances ORDER BY id ASC"),
        settings: await queryRows("SELECT * FROM settings ORDER BY id ASC"),
        audit_logs: await queryRows("SELECT * FROM audit_logs ORDER BY id ASC"),
        notifications: await queryRows("SELECT * FROM notifications ORDER BY id ASC"),
    };
    const backup = {
        version: 1,
...
app.post("/admin/database/import", asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await client.query("TRUNCATE teams, users, leave_requests, holidays, leave_balances, settings, audit_logs, notifications RESTART IDENTITY CASCADE");
      ...
      `, [
            ...
            JSON.stringify({
                ...
                counts: Object.fromEntries(requiredTables.map((table) => [table, backup.tables[table].length])),
            }),
        ]);

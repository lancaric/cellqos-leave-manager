...
function GroupManagement(){
  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,380px)_minmax(0,1fr)]">
    <SettingsForm title={editing ? "Upraviť skupinu" : "Nová skupina"} form={form} setForm={setForm} employees={employees.data?.employees || []} onSave={() => save.mutate()} cancel={editing ? () => { setEditing(null); setForm({ ...emptySettings }); } : undefined} />
    <Card className="min-w-0 space-y-3 p-3 sm:p-4">
      <h2 className="font-semibold">Existujúce skupiny</h2>
      <div className="space-y-3 md:hidden">
        {groupRows.map((group: any) => <div key={group.id} className="min-w-0 space-y-2 rounded-lg border p-3 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-2"><strong className="break-words">{group.name}</strong><Badge variant="secondary">{group.internCount} praktikantov</Badge></div>
          <div><span className="text-muted-foreground">Obdobie:</span> {group.startDate} – {group.endDate}</div>
          <div><span className="text-muted-foreground">Dni:</span> {group.weekdays.map((day: number) => weekdays.find((item) => item.id === day)?.label).join(", ")}</div>
          <div className="break-words"><span className="text-muted-foreground">Poverený:</span> {group.supervisorName}</div>
          {group.substituteName && <div className="break-words"><span className="text-muted-foreground">Zástupca:</span> {group.substituteName}</div>}
          <div className="flex flex-wrap gap-2 pt-1"><Button size="sm" variant="outline" onClick={() => edit(group)}>Upraviť</Button><Button size="sm" variant="destructive" onClick={() => remove.mutate(Number(group.id))}>Odstrániť</Button></div>
        </div>)}
      </div>
      <div className="hidden md:block"><Table><TableHeader><TableRow><TableHead>Skupina</TableHead><TableHead>Obdobie a dni</TableHead><TableHead>Poverený</TableHead><TableHead>Počet</TableHead><TableHead /></TableRow></TableHeader><TableBody>{groupRows.map((group: any) => <TableRow key={group.id}><TableCell>{group.name}</TableCell><TableCell>{group.startDate} – {group.endDate}<br /><span className="text-muted-foreground">{group.weekdays.map((day: number) => weekdays.find((item) => item.id === day)?.label).join(", ")}</span></TableCell><TableCell>{group.supervisorName}{group.substituteName && <><br /><span className="text-muted-foreground">Zástupca: {group.substituteName}</span></>}</TableCell><TableCell>{group.internCount}</TableCell><TableCell><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => edit(group)}>Upraviť</Button><Button size="sm" variant="destructive" onClick={() => remove.mutate(Number(group.id))}>Odstrániť</Button></div></TableCell></TableRow>)}</TableBody></Table></div>
      {groupRows.length === 0 && <p className="text-sm text-muted-foreground">Zatiaľ nie je vytvorená žiadna skupina.</p>}
    </Card>
  </div>;
}
...
function SettingsForm({title,form,setForm,employees,onSave,cancel}:{title:string;form:any;setForm:(v:any)=>void;employees:any[];onSave:()=>void;cancel?:()=>void}){
  return <Card className="min-w-0 space-y-4 p-4">
    <h2 className="font-semibold">{title}</h2>
    <div><Label>Názov</Label><Input value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></div>
    <div className="grid gap-3 sm:grid-cols-2"><div className="min-w-0"><Label>Od</Label><Input type="date" value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})}/></div><div className="min-w-0"><Label>Do</Label><Input type="date" value={form.endDate} onChange={e=>setForm({...form,endDate:e.target.value})}/></div></div>
    <div><Label>Dni praxe</Label><DayPicker value={form.weekdays} onChange={v=>setForm({...form,weekdays:v})}/></div>
    <div><Label>Opakovanie (každý N-týždeň)</Label><Input type="number" min={1} max={52} value={form.weekInterval} onChange={e=>setForm({...form,weekInterval:Number(e.target.value)})}/></div>
    <div><Label>Poverený zamestnanec</Label><select className="h-9 w-full min-w-0 rounded-md border bg-background px-3" value={form.supervisorUserId} onChange={e=>setForm({...form,supervisorUserId:e.target.value})}><option value="">Vyberte...</option>{employees.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
    <div><Label>Zástupca (voliteľné)</Label><select className="h-9 w-full min-w-0 rounded-md border bg-background px-3" value={form.substituteUserId} onChange={e=>setForm({...form,substituteUserId:e.target.value})}><option value="">Bez zástupcu</option>{employees.filter(e=>e.id!==form.supervisorUserId).map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
    <div className="flex flex-wrap gap-2"><Button onClick={onSave}><Save className="mr-2 h-4 w-4"/>Uložiť</Button>{cancel&&<Button variant="outline" onClick={cancel}>Zrušiť</Button>}</div>
  </Card>;
}
...
function InternManagement(){
  ...
  const internRows = interns.data?.interns || [];
  const cancelEdit = () => { setEditing(null); setForm({...emptySettings,email:"",password:"",groupId:"",isActive:true}); };
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,400px)_minmax(0,1fr)]">
    <Card className="min-w-0 space-y-4 p-4">
      <h2 className="font-semibold">{editing ? "Upraviť praktikanta" : "Nový praktikant"}</h2>
      <div><Label>Meno</Label><Input value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></div>
      <div><Label>E-mail (prihlasovacie meno)</Label><Input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></div>
      <div><Label>{editing ? "Nové heslo (ponechajte prázdne bez zmeny)" : "Heslo (min. 8 znakov)"}</Label><Input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/></div>
      <div><Label>Skupina</Label><select className="h-9 w-full min-w-0 rounded-md border bg-background px-3" value={form.groupId} onChange={e=>setForm({...form,groupId:e.target.value})}><option value="">Individuálne nastavenie</option>{(groups.data?.groups||[]).map((g:any)=><option key={g.id} value={g.id}>{g.name}</option>)}</select></div>
      {form.groupId ? <p className="rounded-md bg-muted p-3 text-sm">Obdobie, dni a poverení zamestnanci sa preberajú zo skupiny a nemožno ich upraviť individuálne.</p> : <SettingsFormFields form={form} setForm={setForm} employees={employees.data?.employees||[]}/>}
      {editing && <label className="flex items-center gap-2"><Checkbox checked={form.isActive} onCheckedChange={v=>setForm({...form,isActive:Boolean(v)})}/>Aktívny účet</label>}
      <div className="flex flex-wrap gap-2"><Button onClick={()=>save.mutate()}><Plus className="mr-2 h-4 w-4"/>{editing ? "Uložiť" : "Pridať"}</Button>{editing && <Button variant="outline" onClick={cancelEdit}>Zrušiť</Button>}</div>
    </Card>
    <Card className="min-w-0 space-y-3 p-3 sm:p-4">
      <h2 className="font-semibold">Existujúci praktikanti</h2>
      <div className="space-y-3 md:hidden">
        {internRows.map((intern: any) => <div key={intern.id} className="min-w-0 space-y-2 rounded-lg border p-3 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-2"><strong className="break-words">{intern.name}</strong><Badge variant={intern.isActive ? "default" : "secondary"}>{intern.isActive ? "Aktívny" : "Neaktívny"}</Badge></div>
          <div className="break-all text-muted-foreground">{intern.email}</div>
          <div><span className="text-muted-foreground">Nastavenie:</span> {intern.groupName || "Individuálne"}</div>
          <div><span className="text-muted-foreground">Obdobie:</span> {intern.startDate} – {intern.endDate}</div>
          <div className="break-words"><span className="text-muted-foreground">Poverený:</span> {intern.supervisorName}</div>
          <Button size="sm" variant="outline" onClick={()=>edit(intern)}>Upraviť</Button>
        </div>)}
      </div>
      <div className="hidden md:block"><Table><TableHeader><TableRow><TableHead>Praktikant</TableHead><TableHead>Nastavenie</TableHead><TableHead>Poverený</TableHead><TableHead>Stav</TableHead><TableHead /></TableRow></TableHeader><TableBody>{internRows.map((intern: any)=><TableRow key={intern.id}><TableCell>{intern.name}<br/><span className="text-muted-foreground">{intern.email}</span></TableCell><TableCell>{intern.groupName||"Individuálne"}<br/><span className="text-muted-foreground">{intern.startDate} – {intern.endDate}</span></TableCell><TableCell>{intern.supervisorName}</TableCell><TableCell><Badge variant={intern.isActive?"default":"secondary"}>{intern.isActive?"Aktívny":"Neaktívny"}</Badge></TableCell><TableCell><Button size="sm" variant="outline" onClick={()=>edit(intern)}>Upraviť</Button></TableCell></TableRow>)}</TableBody></Table></div>
      {internRows.length === 0 && <p className="text-sm text-muted-foreground">Zatiaľ nie je pridaný žiadny praktikant.</p>}
    </Card>
  </div>;
}
function SettingsFormFields({form,setForm,employees}:{form:any;setForm:(v:any)=>void;employees:any[]}){
  return <>
    <div className="grid gap-3 sm:grid-cols-2"><div className="min-w-0"><Label>Od</Label><Input type="date" value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})}/></div><div className="min-w-0"><Label>Do</Label><Input type="date" value={form.endDate} onChange={e=>setForm({...form,endDate:e.target.value})}/></div></div>
    <div><Label>Dni praxe</Label><DayPicker value={form.weekdays} onChange={v=>setForm({...form,weekdays:v})}/></div>
    <div><Label>Opakovanie (každý N-týždeň)</Label><Input type="number" min={1} max={52} value={form.weekInterval} onChange={e=>setForm({...form,weekInterval:Number(e.target.value)})}/></div>
    <div><Label>Poverený zamestnanec</Label><select className="h-9 w-full min-w-0 rounded-md border bg-background px-3" value={form.supervisorUserId} onChange={e=>setForm({...form,supervisorUserId:e.target.value})}><option value="">Vyberte...</option>{employees.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
    <div><Label>Zástupca</Label><select className="h-9 w-full min-w-0 rounded-md border bg-background px-3" value={form.substituteUserId} onChange={e=>setForm({...form,substituteUserId:e.target.value})}><option value="">Bez zástupcu</option>{employees.filter(e=>e.id!==form.supervisorUserId).map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
  </>;
}
